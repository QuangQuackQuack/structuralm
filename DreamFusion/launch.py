...
login(token=os.getenv("MY_HUGGINGFACE_TOKEN"))
...
def main(args, extras) -> None:
    ...
    import torch
    torch.serialization.allow_unsafe_globals = True
    import omegaconf  # <-- add

    # --- FIX FOR PYTORCH 2.6+ CHECKPOINT LOADING ---
    # Whitelist OmegaConf container classes so torch.load can unpickle older checkpoints.
    torch.serialization.add_safe_globals([
        omegaconf.listconfig.ListConfig,
        omegaconf.dictconfig.DictConfig,
        omegaconf.base.ContainerMetadata,   # <-- add this
    ])
